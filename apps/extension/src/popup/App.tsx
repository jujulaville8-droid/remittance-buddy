...
export function App() {
  ...
  async function handleCheck() {
    ...
    try {
      ...
      setQuotes([
        { provider: 'Remitly', receiveAmount: amt * 57.98, receiveCurrency, exchangeRate: 57.98, fee: 1.99, deliveryTime: 'Minutes (GCash)', affiliateUrl: 'https://www.remitly.com/us/en/philippines' },
        { provider: 'Wise', receiveAmount: amt * 58.33, receiveCurrency, exchangeRate: 58.33, fee: 4.14, deliveryTime: '1-2 days', affiliateUrl: 'https://wise.com/us/send-money/send-money-to-philippines' },
        { provider: 'Xoom', receiveAmount: amt * 57.57, receiveCurrency, exchangeRate: 57.57, fee: 0, deliveryTime: 'Hours (GCash)', affiliateUrl: 'https://www.xoom.com/philippines/send-money' },
        { provider: 'Western Union', receiveAmount: amt * 56.99, receiveCurrency, exchangeRate: 56.99, fee: 5.00, deliveryTime: 'Minutes (GCash)', affiliateUrl: 'https://www.westernunion.com/us/en/send-money-to-philippines.html' },
        { provider: 'MoneyGram', receiveAmount: amt * 57.28, receiveCurrency, exchangeRate: 57.28, fee: 4.99, deliveryTime: 'Minutes (GCash)', affiliateUrl: 'https://www.moneygram.com/mgo/us/en/send/philippines' },
        { provider: 'WorldRemit', receiveAmount: amt * 57.75, receiveCurrency, exchangeRate: 57.75, fee: 2.99, deliveryTime: 'Minutes (GCash)', affiliateUrl: 'https://www.worldremit.com/en/philippines' },
        { provider: 'Pangea', receiveAmount: amt * 57.87, receiveCurrency, exchangeRate: 57.87, fee: 3.95, deliveryTime: 'Same day (GCash)', affiliateUrl: 'https://www.pangeamoneytransfer.com' },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const sortedByTotalCost = hasQuotes ? [...quotes].sort((a, b) => a.fee - b.fee) : [];
    ...
    <div className="flex flex-col h-full bg-[hsl(var(--background))] overflow-y-auto">
      ...
      <div className="px-4 pb-4 space-y-3">
        ...
        {hasQuotes ? (
          <div>
            {savingsAmount > 0 && (
              <div className="flex items-center gap-2 bg-[hsl(var(--teal-light))] rounded-2xl px-3 py-2 mb-2">
                <TrendingDown className="h-3.5 w-3.5 text-[hsl(var(--teal))] shrink-0" />
                <span className="text-xs font-bold text-[hsl(var(--teal))]">{t('youSave')} ${savingsAmount.toFixed(2)} {t('vsExpensive')}</span>
              </div>
            )}
            <div className="flex items-center justify-between px-1 mb-2">
              <h2 className="text-[10px] font-bold text-[hsl(var(--muted-foreground))] uppercase tracking-widest">{t('results')}</h2>
              <button onClick={() => setQuotes([])} className="text-[11px] font-semibold text-[hsl(var(--coral))] hover:underline">{t('clear')}</button>
            </div>
            <div className="space-y-2 stagger-children">
              {quotes.map((quote, i) => (
                <RateCard key={quote.provider} {...quote} isCheapest={i === 0} savingsAmount={i === 0 ? savingsAmount : undefined} />
              ))}
            </div>
          </div>
        ) : (
